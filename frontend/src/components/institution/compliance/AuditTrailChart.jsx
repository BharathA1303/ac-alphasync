...
import { BarChart3, CheckCircle2, ShieldCheck, Lock } from 'lucide-react';
    ...
    return (
        <div className="rounded-xl bg-surface-900/70 border border-edge/10 p-4 flex flex-col justify-between shadow-sm">
            <div>
                ...
                <div className="flex items-center justify-between pb-3 border-b border-edge/10">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-bold text-heading">
                            Audit trail <span className="text-xs font-normal text-gray-400">— {period}</span>
                        </h3>
                    </div>
                ...
                <div className="h-28 flex items-end gap-1.5 pt-4 pb-2 px-1">
                    {daily_volume.map((item, idx) => {
                        ...
                        return (
                            ...
                            >
                                {/* Tooltip */}
                                <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-8 px-1.5 py-0.5 rounded bg-surface-800 text-[9px] font-mono text-white pointer-events-none whitespace-nowrap shadow-md z-10">
                ...
                {/* Days axis labels */}
                <div className="flex items-center justify-between text-[9px] font-mono text-gray-500 px-1 border-t border-edge/5 pt-1">
            ...
            <div className="grid grid-cols-2 gap-2 pt-3 border-t border-edge/10 mt-3 text-xs">
                <div>
                    <span className="text-[10px] text-gray-500 font-mono block">Records written today</span>
                    <span className="font-mono font-bold text-heading text-[12px]">{records_written_today}</span>
                </div>
                <div>
                    <span className="text-[10px] text-gray-500 font-mono block">Chain head</span>
                    <span className="font-mono font-medium text-gray-300 text-[11px] truncate block">{chain_head_published}</span>
                </div>
                <div>
                    <span className="text-[10px] text-gray-500 font-mono block">Integrity check</span>
                ...
                <div>
                    <span className="text-[10px] text-gray-500 font-mono block">Retention</span>
                    <span className="font-mono font-medium text-gray-300 text-[11px]">{retention}</span>
                </div>
            </div>
        </div>
    );
