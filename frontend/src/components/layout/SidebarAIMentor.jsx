...
import { Bot, Clipboard, Loader2, MessageSquarePlus, Send, Star, Trash2, UserRound, X } from 'lucide-react';
...
const SidebarAIMentor = ({ onClose }) => {
  ...
  return (
    <div className="w-full sm:w-[430px] h-full max-h-screen sm:max-h-[700px] flex flex-col rounded-t-2xl sm:rounded-2xl border border-cyan-200/20 bg-[linear-gradient(140deg,#0a1236_0%,#13285f_45%,#183478_100%)] shadow-2xl overflow-hidden">
          ...
          {filteredMessages.map((msg) => (
            ...
            >
              ...
              <div className="group max-w-[85%] sm:max-w-sm">
                <div
                  className={cn(
                    'p-3 rounded-2xl break-words text-sm leading-relaxed border shadow-lg max-w-full overflow-hidden',
                    msg.type === 'user'
      ...
      >
        <div className="flex items-end gap-1.5 rounded-full border border-white/20 bg-white/10 px-2 py-1">
          <textarea
            ref={inputRef}
            value={inputValue}
            ...
            rows={1}
            className={cn(
              'flex-1 min-h-[32px] max-h-24 resize-none px-2 py-1.5 bg-transparent border-0',
