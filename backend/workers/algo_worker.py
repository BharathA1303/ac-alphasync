...
class AlgoStrategyWorker:
    ...
    async def _monitor_open_positions(self) -> None:
        ...
        for sid, pos in list(self._positions.items()):
            try:
                quote = await market_data.get_quote_safe(pos.symbol, pos.user_id)
    ...
    async def _evaluate_strategy(
        ...
        # Use candle interval/period from strategy params — intraday strategies
        # use 5m bars; swing/positional strategies use 1d daily bars.
        candle_interval = str(parameters.get("candle_interval", "5m")).strip()
        candle_period   = str(parameters.get("candle_period", "5d")).strip()

        # Validate known intervals
        valid_intervals = {"1m", "2m", "5m", "15m", "30m", "60m", "1h", "1d", "1wk"}
        if candle_interval not in valid_intervals:
            candle_interval = "5m"
        valid_periods = {"1d", "5d", "1mo", "3mo", "6mo", "1y", "2y", "5y"}
        if candle_period not in valid_periods:
            candle_period = "5d" if candle_interval != "1d" else "3mo"
        ...
        # Minimum candle requirement: 20 for short-term; 30 for daily
        min_candles = 20 if candle_interval != "1d" else 30
        ...
        if not candles or len(candles) < min_candles:
            db.add(
                AlgoLog(
                    strategy_id=strategy.id,
                    level="WARNING",
                    message=f"Insufficient historical data ({len(candles) if candles else 0} candles, interval={candle_interval}, period={candle_period})",
                )
            )
            return
        ...
        segment = str(parameters.get("segment", "EQUITY")).upper()
        order_tag = f"ALGO_{segment}"
