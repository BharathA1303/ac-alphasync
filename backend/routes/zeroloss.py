...
async def zeroloss_config(
    ...
    if req.segment is not None:
        seg = str(req.segment).strip().upper()
        if seg not in ("EQUITY", "FUTURES", "OPTIONS", "ALL"):
            raise HTTPException(
                status_code=400,
                detail="segment must be one of EQUITY, FUTURES, OPTIONS, ALL",
            )
        controller.set_segment(seg)
        changes.append(f"segment={seg}")
