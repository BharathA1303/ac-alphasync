...
class IndicatorEngine:
    ...
    def adx(
        ...
        Returns dict with keys:
            adx   : list[Optional[float]] — trend strength (>=25 = trending)
            plus_di: list[Optional[float]] — bullish directional indicator
            minus_di: list[Optional[float]] — bearish directional indicator
        """
        n = len(closes)
        if n < period + 2:
            empty = [None] * n
        ...
        def _wilders(values: list[float], p: int) -> list[float]:
            out = [None] * (p - 1)
            s = sum(values[:p])
            out.append(s)
            for v in values[p:]:
        ...
        smoothed_tr = _wilders(tr_list, period)
        smoothed_pdm = _wilders(plus_dm, period)
        smoothed_mdm = _wilders(minus_dm, period)

        plus_di: list[Optional[float]] = []
        minus_di: list[Optional[float]] = []
        dx_list: list[float] = []

        for tr_s, pdm_s, mdm_s in zip(smoothed_tr, smoothed_pdm, smoothed_mdm):
            if tr_s is None or tr_s == 0:
                plus_di.append(None)
                minus_di.append(None)
            else:
                pdi = round(100 * pdm_s / tr_s, 2)
                mdi = round(100 * mdm_s / tr_s, 2)
                plus_di.append(pdi)
                minus_di.append(mdi)
                di_sum = pdi + mdi
                if di_sum > 0:
        ...
        adx_raw: list[Optional[float]] = []
        if len(dx_list) >= period:
            adx_smooth = _wilders(dx_list, period)
            # pad front to align with plus_di / minus_di
            none_pad = n - len(adx_smooth)
            adx_raw = [None] * none_pad + [round(v, 2) if v is not None else None for v in adx_smooth]
        else:
            adx_raw = [None] * n

        return {"adx": adx_raw, "plus_di": plus_di, "minus_di": minus_di}

    @staticmethod
    def rolling_high(prices: list[float], period: int) -> list[Optional[float]]:
        """N-bar rolling highest value."""
        if len(prices) < period:
            return [None] * len(prices)
        result: list[Optional[float]] = [None] * (period - 1)
        for i in range(period - 1, len(prices)):
            result.append(max(prices[i - period + 1 : i + 1]))
        return result

    @staticmethod
    def rolling_low(prices: list[float], period: int) -> list[Optional[float]]:
        """N-bar rolling lowest value."""
        if len(prices) < period:
            return [None] * len(prices)
        result: list[Optional[float]] = [None] * (period - 1)
        for i in range(period - 1, len(prices)):
            result.append(min(prices[i - period + 1 : i + 1]))
        return result
